...
def trainModel (model, Tokenized_TextDataset, name_run):
    ...
    trainer = Seq2SeqTrainer(
        model=model,
        args=training_args,
        train_dataset=Tokenized_TextDataset["train"],
        eval_dataset=Tokenized_TextDataset["test"],
        tokenizer=tokenizer,
...
#TextDataset = cnn["test"].train_test_split(test_size=0.99)
#Tokenized_TextDataset = TextDataset.map(preprocess_function, batched=True)
Tokenized_TextDataset = cnn.map(preprocess_function, batched=True)
model = trainModel (model, Tokenized_TextDataset, name_run+"_CNN")
